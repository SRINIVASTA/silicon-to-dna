...
def check_homopolymers(dna_sequence):
    ...
    for i in range(1, len(dna_sequence)):
        if dna_sequence[i] == dna_sequence[i-1]:
            ...
            current_run = 1
    return max(max_run, current_run) if len(dna_sequence) > 0 else 0
...
with tab1:
    ...
    uploaded_file = st.file_uploader("Choose a small file (Text, Image, PDF up to 50KB):", type=["pdf", "png", "jpg", "txt"], label_visibility="collapsed")
    ...
    if uploaded_file is not None:
        ...
        if file_size_kb > 50.0:
            ...
            st.text_area("Full Untruncated Sequence Stream", value=dna_sequence, height=120, disabled=True, label_visibility="collapsed")
